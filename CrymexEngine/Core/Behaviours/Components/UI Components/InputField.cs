...
namespace CrymexEngine.UI
{
    public class InputField : UIComponent, IMouseClick, IMouseHover
    {
        ...
        {
            _displayText = new TextObject(Element.Transform.Position, VectorUtil.RoundToInt(Element.Transform.Scale), "", Assets.DefaultFontFamily, (int)(Element.Transform.Scale.Y * 0.8f), Alignment.MiddleLeft);
            _displayText.BestFit = true;
            _displayText.TextPadding = new Vector2(10, 0);
            _displayText.MaxBestFitSize = (int)(Element.Transform.Scale.Y * 0.8f);
